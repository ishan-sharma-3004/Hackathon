...
index_client = SearchIndexClient(endpoint=search_service_endpoint, credential=AzureKeyCredential(search_service_key))
...
class VectorField:
    ...
    def _to_generated(self):
        return {
            ...
            "vector_search_configuration": {"name": self.vector_search_configuration.name}
        }
index = SearchIndex(
    name=index_name,
    fields=[
        ...
        VectorField(name="embedding", field_type="Collection(Edm.Single)", dimensions=1536,
                    vector_search_configuration=VectorSearchAlgorithmConfiguration(name="my-vector-algorithm"))
    ]
)
...
def vector_search():
    try:
        ...
        results = search_client.search(
            search_text="",
            vector={"field": "embedding", "value": query_embedding, "k": 5}  # Adjust 'k' as needed
        )

        response_content = [result['content'] for result in results if 'content' in result]
